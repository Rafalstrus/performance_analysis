...
class Checker
{
    ...
    private static ?bool $isJitEnabled = null;
    ...
    {
        if (is_null(self::$isJitEnabled)) {
            self::$isJitEnabled = isset(opcache_get_status()["jit"]["enabled"]) && opcache_get_status()["jit"]["enabled"];
        }
        ...
        $textToAppend .=  't: ' . $time . ', m: ' . $memory . PHP_EOL;

        file_put_contents(__DIR__ . '/result/' . $path . '/' . $script . '.txt', $textToAppend, FILE_APPEND);
    }
}
